...
export default function PostPage() {
  ...
  const unSlugTitle = async (title) => {
    ...
    for (let i = 0; i < data.topics.length; i++) {
      console.log(slugify(data.topics[i]));
      if ( slugify(data.topics[i]) === title) {
        setTopic(data.topics[i]);
        break;
      }
    }
  ...
  return (
     ...
     <div className="max-w-screen-2xl mx-auto flex flex-col md:flex-row gap-8 px-6 py-8">
      ...
      <h1 className="!text-3xl font-bold text-blue-700 !flex justify-left" >{topic}</h1>
