...
export default function AdmissionResults() {
  ...
  const [hasResults, setHasResults] = useState(new Date() >= resultAnnouncementDate); // Có kết quả nếu thời gian hiện tại >= ngày công bố
  ...
  // Khởi tạo timeLeft
  const initialTimeLeft = calculateTimeLeft(resultAnnouncementDate);
  ...
  useEffect(() => {
    if (!hasResults) {
      const interval = setInterval(() => {
        const timeRemaining = calculateTimeLeft(resultAnnouncementDate);
        setTimeLeft(timeRemaining);
        setAnimatedNumber(timeRemaining.days); // Cập nhật số ngày cho hiệu ứng
        ...
        if (
          ...
          setHasResults(true); // Chuyển sang giao diện kết quả
  ...
  // Nếu chưa có kết quả, hiển thị giao diện đếm ngược
  if (!hasResults) {
    return (
          ...
          <div className="w-full md:w-1/2">
            <div className="bg-white rounded-lg shadow-lg p-6 h-full transform hover:translate-y-1 transition-transform duration-300">
              ...
              <p className="text-gray-600">
                Kết quả đang được xử lý và sẽ được công bố theo thời gian quy định. Vui
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 h-full border-l-4 border-blue-500">
              ...
              <div className="mb-4">
                <details className="group border-b border-gray-200 py-2">
                  ...
                  <p className="text-gray-600 mt-2 pl-4">
                    Bạn nên chuẩn bị sẵn giấy CMND/CCCD, giấy khai sinh, học bạ, bằng tốt
                    nghiệp THPT (hoặc giấy chứng nhận tốt nghiệp tạm thời) và các giấy tờ
                    ưu tiên (nếu có).
                  </p>
                </details>
                <details className="group border-b border-gray-200 py-2">
                  ...
                  <p className="text-gray-600 mt-2 pl-4">
                    Sau khi có kết quả trúng tuyển, bạn cần xác nhận nhập học trên Cổng
                    thông tin xét tuyển của Bộ GD&ĐT và làm theo hướng dẫn nhập học của
                    trường.
                  </p>
                </details>
              </div>
              ...
              <div className="mt-4 flex items-center justify-start bg-gradient-to-r from-blue-50 to-white p-3 rounded-lg transform hover:scale-105 transition-all duration-300">
                <div className="relative">
                  <svg
                    className="h-6 w-6 text-blue-600 animate-ping absolute opacity-75"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
                ...
                <div className="ml-3">
                  <p className="text-blue-700 font-semibold">Hỗ trợ 24/7</p>
                  <p className="text-blue-600">
                    Mọi thắc mắc vui lòng liên hệ hotline:{' '}
                    <span className="font-bold">1900 1508</span>
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="w-full bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-lg shadow-md flex items-start mb-6 transform transition-transform duration-300 hover:scale-105">
          ...
          <div>
            <h3 className="font-semibold text-yellow-800">
              Lưu ý về thời hạn xác nhận nhập học!
            </h3>
            <p className="text-yellow-700">
              Sau khi có kết quả trúng tuyển, bạn cần xác nhận nhập học trong vòng 5
              ngày qua Cổng thông tin tuyển sinh của Bộ GD&ĐT. Quá thời hạn này sẽ được
              xem như từ chối nhập học.
            </p>
          </div>
        </div>
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6 flex items-center justify-start">
        ...
        <div>
          ...
          <p className="text-green-600">
            Vui lòng hoàn thiện thủ tục xác nhận nhập học trực tuyến công xét tuyển của
            Bộ GD&ĐT trước 17:00 ngày 15/08/2024
          </p>
        </div>
      </div>
